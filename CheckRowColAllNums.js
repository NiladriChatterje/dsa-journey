...
An n x n matrix is valid
 if every row and every column 
 contains all the integers from 1 to n
  (inclusive).

Given an n x n integer matrix matrix, 
...
var checkValid = function(matrix){
  ...
  for(let i in matrix)
    {let temp=[]
      ...
      matrixTrans.push(temp);
    }

  for(let i=0;i<matrix.length;i++)
    for(let j=0;j<matrix.length;j++)
    if(!(matrix[i].includes(j+1) && matrixTrans[i].includes(j+1))) return false;    
...
console.log(checkValid([[4,4,3,5,2],[3,4,3,4,1],[2,2,2,1,3],[5,2,1,3,4],[1,2,1,2,5]]));
console.log(checkValid([[15,7,18,11,19,10,14,16,8,2,3,6,5,1,17,12,9,4,13],
